...
void Pigeon::init() {
  for (int i = 0; i < 10; i++) {
    ...
    if (!send_with_ack("\xB5\x62\x06\x01\x03\x00\x01\x35\x01\x41\xAD"s)) continue;
...
void PandaPigeon::set_baud(int baud) {
  panda->control_write(0xe2, 1, 0);
  panda->control_write(0xe4, 1, baud/300);
}
...
std::string PandaPigeon::receive() {
  ...
  while (r.length() < 0x1000) {
    int len = panda->control_read(0xe0, 1, 0, dat, sizeof(dat));
    if (len <= 0) break;
...
void PandaPigeon::set_power(bool power) {
  panda->control_write(0xd9, power, 0);
}

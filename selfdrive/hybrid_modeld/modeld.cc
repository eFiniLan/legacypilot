...
#include "selfdrive/hybrid_modeld/models/nav.h"
...
//  double last = 0;
  ...
  bool is_rhd = Params().getBool("IsRhdDetected");
  ...
  while (!do_exit) {
    ...
    if (sm.updated("liveCalibration")) {
      ...
      model_transform_extra = update_calibration(device_from_calib_euler, false, true);
